<template>
  <div class="w-full h-full overflow-y-auto">
    ...
    <header class="flex items-center justify-between border-b border-gray-100 shadow-sm h-20 px-6">
      <div class="w-full">
        <h2 class="text-2xl text-gray-700">Installments Entries</h2>
        <p class="text-gray-400 text-sm">All emi's List</p>
      </div>
    </header>
                  ...
                  <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                      <tr 
                        ...
                        v-for="installment in installments" :key="installment"
                      >
                        <td class="size-px whitespace-nowrap align-center">
                          <button class="block p-4">
                            <span class="block text-sm text-gray-800 dark:text-neutral-200">{{installment.loans.file_number}}</span>
                          </button>
                        </td>
                        <td class="size-px whitespace-nowrap align-center">
                          <button class="block p-4">
                            <span class="block text-sm text-gray-800 dark:text-neutral-200">{{installment.loans.client_name}}</span>
                          </button>
                        </td>
                        <td class="size-px whitespace-nowrap align-center">
                          <button class="block p-4 text-sm">
                            ₹ {{installment.loans.loan_amount}}
                          </button>
                        </td>
                        <td class="size-px whitespace-nowrap align-center">
                          <button class="block p-4">
                            <span class="block text-sm text-gray-500 dark:text-neutral-500">₹ {{installment.emi}}</span>
                          </button>
                        </td>
                        <td class="size-px whitespace-nowrap align-center">
                          <button class="block p-4">
                            <span class="text-sm text-gray-600 dark:text-neutral-400">{{installment.date}}</span>
                          </button>
                        </td>
                        <td class="size-px whitespace-nowrap align-center text-center">
                          <UToggle
                            color="green"
                            on-icon="i-heroicons-check-20-solid"
                            off-icon="i-heroicons-x-mark-20-solid"
                            :model-value="installment.status"
                          />
                        </td>
                      </tr>
...
<script setup>
  ...
  const fetchRecords = async () => {
    let { data, error } = await supabase
      .from('installments')
      .select('*, loans(client_name, file_number, loan_amount)')
