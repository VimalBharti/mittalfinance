<template>
  <div class="w-full h-full overflow-y-auto relative">
              ...
              <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="bg-white overflow-hidden dark:bg-neutral-900 dark:border-neutral-700">
                  ...
                  <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700 bg-red-300">
                      <tr 
    ...
    <!-- All Loans by Client -->
     <div class="absolute top-0 w-full bottom-0 bg-gray-500/25 backdrop-blur flex items-center justify-center p-0" v-if="isOpen">
        <section class="w-full mx-auto h-full overflow-y-auto bg-white p-6">
          <div class="header flex items-center justify-between bg-gray-100 p-3 mb-4 rounded-lg">
            <h2>All Loans <strong>({{ loansCount }})</strong></h2>
            <button @click="isOpen = false"><Icon name="material-symbols:close-rounded" class="w-5 h-5" /></button>
          </div>
          <div class="grid grid-cols-2 gap-5">
            <ul class="bg-white flex flex-col text-xs" v-for="loan in loansByClient" :key="loan">
...
<script setup>
  ...
  const singleClientDetail = async (select) => {
    let { data: loans, count: total, error } = await supabase
      .from('loans')
      .select("*", { count: 'exact' })
      .eq('client_id', select.uid)

    loansCount.value = total
    loansByClient.value = loans
    isOpen.value = true
  }
